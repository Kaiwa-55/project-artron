...
func execute_ground_skill(combatant_id: String, skill_id: String, target_point: Vector2) -> ActionResult:
	var start_validation := validate_ground_skill_start(combatant_id, skill_id)
	if not start_validation.success:
		return start_validation
	var actor: CombatantState = combat_state.get_combatant(combatant_id)
	var skill = get_ground_skill(actor, skill_id)
	var point_validation: ActionResult = targeting_system.validate_target_point(actor, target_point, skill, map_rules)
	if not point_validation.success:
		return point_validation
	var targets: Array[CombatantState] = targeting_system.collect_targets(actor, target_point, skill, combat_state, map_rules)
	if targets.is_empty():
	...
	pending_area_context = AreaActionContextScript.new()
	pending_area_context.setup_skill(actor, skill, target_point, area_attack, targets)
	pending_area_context.costs_consumed = true
	pending_area_context.cooldown_started = skill_system.get_remaining_cooldown(actor, skill.id) > 0
	return continue_area_action()


func execute_ground_ability(combatant_id: String, ability_id: String, target_point: Vector2) -> ActionResult:
	var start_validation := validate_ground_ability_start(combatant_id, ability_id)
	if not start_validation.success:
		return start_validation
	var actor: CombatantState = combat_state.get_combatant(combatant_id)
	var ability = ability_system.get_available_ability(actor, ability_id)
	var point_validation: ActionResult = targeting_system.validate_target_point(actor, target_point, ability, map_rules)
	if not point_validation.success:
		return point_validation
	var targets: Array[CombatantState] = targeting_system.collect_targets(actor, target_point, ability, combat_state, map_rules)
	if targets.is_empty():
		return ActionResult.failure("There are no valid targets in the selected area.")
	var source_attack: AttackData = ability_system.get_attack_data(actor, ability)
	var unarmed_validation: ActionResult = equipment_system.validate_unarmed_attack(actor, source_attack)
	if not unarmed_validation.success:
		return unarmed_validation
	...
	pending_area_context = AreaActionContextScript.new()
	pending_area_context.setup_ability(actor, ability, target_point, area_attack, targets)
	pending_area_context.costs_consumed = true
	pending_area_context.cooldown_started = cooldown > 0
	var ground_result := continue_area_action()
	if ability.faith_cost > 0:
		...
		ground_result.events.push_front(faith_event)
		event_system.emit(faith_event)
...
func validate_ground_skill_start(combatant_id: String, skill_id: String) -> ActionResult:
	...
	var cooldown: int = skill_system.get_remaining_cooldown(actor, skill.id)
	if cooldown > 0:
...
func validate_ground_ability_start(combatant_id: String, ability_id: String) -> ActionResult:
	...
	return ability_system.validate_active_use(actor, ability)


func continue_area_skill(carried_events: Array[CombatEvent] = []) -> ActionResult:
	return continue_area_action(carried_events)
...
func continue_area_action(carried_events: Array[CombatEvent] = []) -> ActionResult:
	...
	if pending_area_context == null:
		return ActionResult.failure("There is no Area Skill waiting to resolve.")
	var context = pending_area_context
	...
	while context.has_remaining_targets():
		...
		if target == null or target.is_dying():
			...
			continue
		var prepared: AttackResult = attack_system.resolve_attack(actor, target, attack, true)
		var defense_prompt: Dictionary = reaction_system.get_post_hit_prompt(actor, target, attack, prepared, combat_state.current_round) if attack.requires_to_hit or attack.base_damage > 0 else {}
		if not defense_prompt.is_empty():
			if target.id == "player":
				...
				if not open_reaction_prompt(area_request, defense_prompt):
					attack_system.finalize_attack(actor, target, attack, prepared)
					result.events.append_array(action_system.build_attack_result(actor, target, attack, prepared).events)
					continue
		...
		var target_events: Array[CombatEvent] = action_system.build_attack_result(actor, target, attack, prepared).events
		result.events.append_array(target_events)
		if ability != null:
			apply_active_ability_effects(actor, target, ability, target_events, result.events)
		if actor.is_dying():
			context.cancel(AreaActionContextScript.CancelScope.REMAINING_TARGETS, "The Area Action user became Dying during resolution.")
			break
	context.finish()
	if skill != null:
		result.events.push_front(CombatEvent.new(EventTypes.Type.SKILL_CAST, actor.id, "", {"skill_name": skill.display_name, "mana_cost": skill.mana_cost, "cooldown": skill_system.get_effective_cooldown_turns(actor, skill), "area_target_count": context.targets.size(), "area_resolved_count": context.target_results.size(), "target_point": context.target_point}))
	else:
...
func resolve_pending_reaction(reaction_index: int) -> ActionResult:
	...
	if reaction != null and reactor != null and reactor.id != "player" and not reactor.is_dying() and movement_effect != null:
	...
	if prompt.get("area_action_continuation", false) or prompt.get("area_skill_continuation", false):
		if pending_area_context != null and prompt.has("prepared_attack"):
			pending_area_context.record_target_result(reactor, prompt["prepared_attack"])
			if pending_area_context.ability_data != null:
				apply_active_ability_effects(pending_area_context.actor, reactor, pending_area_context.ability_data, action_result.events, result.events)
		return continue_area_action(result.events)
