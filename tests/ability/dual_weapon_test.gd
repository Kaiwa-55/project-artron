...
func _init() -> void:
	...
	check(system.ability_system.get_active_abilities(actor).any(func(ability): return ability.id == "dual_strike"), "Dual Strike should appear in the active Ability list.", failures)
	...
	check(actor.ap == 2, "Dual Strike should spend 2 AP once.", failures)
