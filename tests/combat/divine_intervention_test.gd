...
func _init() -> void:
	...
	check(not manual_use.success and devotee.ap == 4 and devotee.faith == devotee.max_faith, "Divine Intervention cannot be activated manually or target an Enemy")
	system.combat_state.current_actor_id = enemy.id
	enemy.ap = 4
	devotee.ap = 4
	var faith_before_reaction: int = devotee.faith
	...
	check(devotee.ap == 3 and devotee.faith == faith_before_reaction - 2, "Divine Intervention costs 1 AP and 2 Faith")
